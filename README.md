...
6. Postgres-Backend aufsetzen:
   1. Auf [Supabase](https://supabase.com) einen Account anlegen.
   2. Projekt erstellen
   3. SQL Statements in `web/db/db-schema.sql` im Supabase Projekt ausführen.
   4. Supabase-URL und -Key in `web/env-template.json` einfügen und umbenennen auf `env.json`.

7. Haiku in Datenbank einspielen:
   1. In Supabase die RLS (Row Level Security) Policies für die `haiku` Tabelle abdrehen.
   2. `python web/db/seed-haikus.py`
   3. RLS wieder aufdrehen!
